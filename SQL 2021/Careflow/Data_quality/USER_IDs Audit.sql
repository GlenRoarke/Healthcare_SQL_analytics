...
,CP.PAS_ID
,ISNULL(MERGE_PAT_NUMBER,'') [Merged into PAS_ID]
,(CASE MINOR_PAT_FLAG
		WHEN 'Y' THEN 1
		ELSE 0
		END) Merged
...
FROM [HDM].[dbo].[FACT_CURRENT_PATIENT] CP WITH (NOLOCK) 

	Left outer join (
			SELECT cp2.PAS_ID, MIN(AF2.CREATE_DTTM) as CREATE_DTTM, SU2.USERS_NAME
		FROM [HDM].[dbo].[FACT_CURRENT_PATIENT] CP2 WITH (NOLOCK) 

		inner join HDM.dbo.FACT_AUDIT_FULL AF2 with(nolock) 
						ON CP2.FACT_CURRENT_PATIENT_ID=AF2.DIM_PATIENT_ID
		inner join HDM.dbo.DIM_AUDIT_TRANSACTION_COMMAND ATC2 with(nolock)
						on ATC2.DIM_AUDIT_TRANSACTION_COMMAND_ID = AF2.DIM_AUDIT_TRANSACTION_COMMAND_ID and ATC2.TRANSACTION_COMMAND ='Save Patient Demographics'
	...
	inner join HDM.dbo.FACT_AUDIT_FULL AF with(nolock) 
						ON CP.FACT_CURRENT_PATIENT_ID=AF.DIM_PATIENT_ID and AF.CREATE_DTTM = A.CREATE_DTTM
	...
	inner join HDM.dbo.DIM_AUDIT_TRANSACTION_COMMAND ATC with(nolock) 
						...
						and (ATC.TRANSACTION_COMMAND ='Save Patient Demographics' or ATC.TRANSACTION_COMMAND = 'Unknown')
...
GROUP BY SU.USERS_NAME, CP.REG_DTTM, AF.CREATE_DTTM, CP.PAS_ID, ENT.DESCRIPTION, ATC.TRANSACTION_COMMAND, MINOR_PAT_FLAG, MERGE_PAT_NUMBER
