...
--This sub query is bringing back searches as well as saved patients

	Left outer join (
			SELECT cp2.PAS_ID, MIN(AF2.CREATE_DTTM) as CREATE_DTTM, SU2.USERS_NAME -- GR I think log date is needed here not the date the audit was created. The report brings back the same Patient ID for differ Usernames but the same date
		...
		Where  (ENT2.DESCRIPTION = 'PATIENT' or ENT2.DESCRIPTION = 'PERSON ID') --i think this or clause will let in other transactions 
	...
	inner join HDM.dbo.DIM_AUDIT_TRANSACTION_COMMAND ATC with(nolock) 
						...
						and (ATC.TRANSACTION_COMMAND ='Save Patient Demographics' or ATC.TRANSACTION_COMMAND = 'Unknown'or ATC.TRANSACTION_COMMAND = 'Process A28') --GR duplication of code here from sub above why is this needed twice?

	INNER JOIN HDM.dbo.DIM_LOOKUP_ENTITY ENT WITH (NOLOCK)   
						ON ENT.DIM_LOOKUP_ENTITY_ID=AF.DIM_LOOKUP_ENTITY_ID  
						and (ENT.DESCRIPTION = 'PATIENT' or ENT.DESCRIPTION = 'Person Id') --what purpose is the or for this entity can they both be for registering patients
...
and convert(date,CP.REG_DTTM)=convert(date,AF.CREATE_DTTM) -- would the audit create date and reg. also the first registered audit would be logged dttm 
